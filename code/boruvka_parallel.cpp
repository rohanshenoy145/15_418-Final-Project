...
vector<Edge> MST(Graph &G){
    ...
    while (G.nodes.size() > 1) {
        ...
        #pragma omp parallel
        {
            ...
            for (size_t i = 0; i < G.edges.size(); i++) {
                Edge cur = G.edges[i];
                if (cur.w < local_shortest[(ithread*rounded_size) + cur.u].weight) {
                    local_shortest[(ithread*rounded_size) + cur.u].index = i; 
                    local_shortest[(ithread*rounded_size) + cur.u].weight = cur.w;
                    local_shortest[(ithread*rounded_size) + cur.u].label = cur.label;
                }
            }
            ...
            for (size_t i = 0; i < G.edges.size(); i ++ ) {
                for (int t = 0 ; t < nthreads; t++ ) {
                    if (local_shortest[(rounded_size*t) + G.edges[i].u].weight < shortest_edges[ G.edges[i].u].weight) {
        ...
        #pragma omp parallel
        {
            ...
            for (size_t i = 0; i < G.nodes.size(); i ++ ) { 
                size_t u = G.nodes[i];
                Edge shortest_from_u = G.edges[shortest_edges[u].index];
                int id = shortest_edges[u].label; //edge id
                size_t v = shortest_from_u.v;
                Edge shortest_from_v = G.edges[shortest_edges[v].index];
    ...
    for(size_t i = 0; i < initial_edges.size(); i++ )
    {
        if(select_edges[i])
        {
            mst_edges[prefix_sum[i] - 1] = initial_edges[i];
        }
    }
